...
export default async function StaffFamilyFile({ params }: { params: { familyId: string } }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link href="/staff/families" className="text-sm text-brand-600">← My families</Link>
        <h1 className="mt-1 text-xl font-bold text-brand-900">{family.name}</h1>
        <p className="text-sm text-brand-600">Read-only family file. Parents log progress; you can review it here.</p>
      </div>
