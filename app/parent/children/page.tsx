...
export default async function ChildrenPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link href="/parent" className="text-sm text-brand-600">
          ← Home
        </Link>
      ...
      <ul className="space-y-2">
          ...
          <li key={c.id}>
            <Link href={`/parent/children/${c.id}`} className="card block hover:border-brand-300">
              <div className="flex items-center justify-between">
                ...
                <span className="pill bg-brand-50 text-brand-600">Edit profile →</span>
              </div>
        ...
        {!children.length && <li className="card text-sm text-brand-500">No children added yet.</li>}
      </ul>
      ...
      <form action={addChildAction} className="card space-y-3">
        ...
        <p className="text-xs text-brand-500">We store the minimum needed — no full legal name required.</p>
